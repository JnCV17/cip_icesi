...
@ManagedBean(name = "AlertRecoleccionController")
@SessionScoped
public class AlertRecoleccionController {
	...
	public AlertRecoleccionController() {
		descripcion = "descripcion correo";
	}

	public String getDescripcion() {
		ParamSmc mensaje = businessDelegate.findCycleActiveByProgram("AlertfechaRecoleccion");
	...
	public void verificar() {
		...
		if (!descripcion.contains("{Subject}")) {
			...
			FacesUtils.addErrorMessage("", "falto la etiqueta  {AssessmentSource}");
			System.out.println("falto AssessmentSource");
		} else if (!descripcion.contains("{Date}")) {
			FacesUtils.addErrorMessage("", "falto la etiqueta  {Date}");
			System.out.println("falto date");
		} else {
			try {
				businessDelegate.updateAlertFechaRecoleccion(descripcion);
