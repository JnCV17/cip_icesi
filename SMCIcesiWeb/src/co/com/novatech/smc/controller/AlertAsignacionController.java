...
@ManagedBean(name = "AlertAsignacionController")
@SessionScoped
public class AlertAsignacionController {
	...
	public AlertAsignacionController() {
		descripcion = "descripcion correo";
	}

	public String getDescripcion() {
		ParamSmc mensaje = businessDelegate.findCycleActiveByProgram("AlertAsignacion");
	...
	public void verificar() {
		...
		if (!descripcion.contains("{Subject}")) {
			...
			// msg.setSeverity(FacesMessage.SEVERITY_ERROR);
			FacesUtils.addErrorMessage("", "falto la etiqueta {Autor}");

			System.out.println("falto Autor");
		} else {
			try {
				businessDelegate.updateAlertAsignacion(descripcion);

				FacesUtils.addInfoMessage("", "Su mensaje se actualizo satisfactoriamente");
			} catch (Exception e) {
				FacesUtils.addErrorMessage("", "error actualizando");
